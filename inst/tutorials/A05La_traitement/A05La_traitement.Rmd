...
Affichez ensuite les 6 premières lignes du tableau (`head()`).
...
- Garder uniquement les individus mâles du jeu de données dont la longeur de la carapace est supérieur à 25 mm (variables `sex` et `length`)
...
crabs ___
  ___(___, ___) ___
  ___(___, ___, ___) ___
  ___(___, mean = ___, number = ___) ___
  ___(___)
```
...
crabs %>.%
  ...
  summarise(___, mean = ___, number = ___) ___
  knitr::kable(___)
```
