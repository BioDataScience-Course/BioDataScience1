...
Dans le tutoriel précédent, vous avez appris les bases du langage R. Vous avez entre autres effectué des calculs, utilisé des fonctions ou encore assigné des résultats. Ce tutoriel va vous permettre d'utiliser le logiciel R afin de résoudre une question biologique sur l'obésité. Vous serez guidé tout au long de ce travail.
...
Nous débutons par sélectionner des colonnes d'intérêt du tableau initial `biometry`.

```{r, echo = TRUE}
bio %<-% sselect(biometry, -c(day_birth, wrist, year_measure))
```

**Notez ceci :**

Nous réalisons cette sélection avec la fonction `sselect()` en éliminant des colonnes du tableau que nous n'utiliserons pas (les variables `day_birth`, `wrist` et `year_measure`). Le résultat de cette fonction est assigné à `bio` grâce à une assignation alternative `%<-%`.
...
Le pipe `%>.%` du package {svFlow} et le chainage sont employés dans les explications ci-dessus.
...
Avez-vous remarqué la ressemblance dans la syntaxe employée pour réaliser un graphique en barres, un histogramme, un graphique en violon ou encore un nuage de points ? La syntaxe est similaire.

On utilise la fonction `chart()`. On commence par lui fournir le tableau de données à employer avec `data =`, puis une formule. Si on reprendre le nuage de points réalisé, on utilise le tableau `biometry` pour réaliser un graphique de la hauteur en fonction (symbolisé par le tilde) de la masse. On spécifie que ce graphique est un nuage de points grâce à la fonction `geom_point()`. La formule peut être plus complexe si on souhaite ajouter des éléments en plus (comme différencier le genre des personnes grâce à la couleur). Le `+`permet d'ajouter des couches sur le graphique. Ici, on souhaite modifier les couleurs par défaut avec la fonction `scale_color_viridis_d()`. En résumé, on aura la structure ci-dessous. La fonction geom\_...() va varier en fonction du type de graphique à réaliser geom_point() (un nuage de points), geom_bar() (un graphique en barres), geom_histogram (un histogramme), geom_boxplot() (une boite de dispersion)...
...
Nous allons maintenant avancer pas à pas dans cette première analyse avec R. Le but est de calculer l'IMC (`BMI`), et puis de visualiser comment cet indice se répartit dans la population étudiée.
...
```{r id2_imc-check}
grade_code("Bien joué ! Vous venez de définir l'indice de masse corporelle de cet homme. Il est en surpoids selon l'échelle de l'OMS")
```
...
Le calcul un à un de l'IMC de chaque individu deviendra très rapidement fastidieux. R permet de créer des vecteurs que l'on peut affecter à des symboles avec la flèche `<-` (constituée du signe plus petit qu’immédiatement suivi du signe moins) comme le montre l'exemple ci-dessous. Pour rassembler plusieurs valeurs dans un vecteur on les "concatène" avec la fonction `c()` :
...
```{r vec_imc_h2-check}
grade_code("Bien joué ! Vous obtenez les 5 valeurs d'IMC en une seule fois avec l'utilisation de vecteurs.")
```

Dans l'exercice ci-dessus, vous avez un bouton `Hints` qui remplace le bouton `Solution` de l'exercice précédent. Les indices sont là pour vous guider dans l'exercice en cas de besoin. Il se peut que la dernière aide soit la solution (cela sera indiqué dans l'aide précédente afin d'éviter de perdre des points en l'affichant de manière malencontreuse). Toute aide affichée fait perdre 10% de la note pour l'exercice. Essayez d'abord de trouver la réponse par vous-même.

## Encodage d'un tableau de données

Il devient rapidement évident qu'il est plus simple que nos observations de terrain soient rassemblées en un jeu de données structuré. Pour cela vous allez créer ce qu'on appelle un **"data frame"** (qui se traduit en français par "tableau de données") dans R. La fonction qui permet de le créer est `dtx()`. Cette dernière permet de combiner vos différents vecteurs colonne par colonne dans un tableau.

Dans `dtx()`, vous entrerez vos différents vecteurs comme autant d'**arguments** de la fonction, séparés par une `,`. De plus, vous pouvez nommer vos colonnes en donnant des noms aux arguments de type `nom = valeur`. Analysez avec attention l'exemple ci-dessous.

```{r, echo=TRUE}
# Création du tableau de données (data frame) avec dtx()
woman <- dtx(
...
Calculez de nouveau l'IMC et ajoutez vos résultats dans le tableau de données. Vous avez à votre disposition la fonction `smutate()` qui requiert comme argument le jeu de données et le nom de la nouvelle variable comme nom d'argument suivi de la formule mathématique qui calcule son contenu.
...
```{r imc_mutate-check}
grade_code("Bien joué ! Vous avez employé la fonction `smutate()` judicieusement.")
```
...
C'est à vous de jouer ! Calculez l'IMC sur le jeu de données `biometry` tout entier. Utilisez la fonction `smutate()`. Nommez cette nouvelle variable `bmi` et assignez votre résultat à `biometry`
...
```{r bio_imc_h2-check}
grade_code(
  correct = "Bien joué ! Vous progressez très rapidement.", incorrect = "Observez la structure proposé sur bio_100. N'auriez vous pas oublié quelques choses ?")
```
...
Votre nouvelle variable comprend une succession de lignes complétées par des chaines de caractères. Organisez cette variable de votre tableau de données avec la fonction `factor()`. Avec l'argument `levels`, vous spécifiez l'ordre des différents niveaux de votre variable à classer. Pour calculer une nouvelle variable, on utilise la fonction `smutate()` comme précédemment.

Classez votre nouvelle variable avec le jeu de données `bio_100`.
...
```{r obese1-check}
grade_code("Vous apprenez vite. Les opérateurs de condition comme `==` sont très utiles en combinaison avec `sfilter()` pour sélectionner les lignes d'intérêt dans le tableau de données.")
```
