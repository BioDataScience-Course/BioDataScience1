...
N'entamer ce tutoriel qu'après avoir compris ce qu'est une ANOVA à un facteur présentée dans le [module 10](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2022/variance.html) du cours et vous être auto-évalué via le learnr **A10La_anova** intitulé "ANOVA à un facteur".
...
plant <- dtx(
  group = as.factor(rep(c("cont", "trt1", "trt2"), each = 30)), 
  yield = c(
    rnorm(30, 60, 2), # groupe "cont" : 30 réplicats, moyenne de 6, écart-type de 2
    rnorm(30, 60.5, 2), # groupe "trt1" : 30 réplicats, moyenne de 10, écart-type de 2
    rnorm(29, 70, 5), 85) # groupe "trt2" : 30 réplicats, moyenne de 15, écart-type de 5 et une valeur extrême
)
```
...
```{r boxplot_h2-check}
grade_code("Les boites de dispersion parallèles sont tout indiquées pour représenter les données avant un test de Wilxoxon indépendant ou un test de Kruskal-Wallis.")
```
...
Votre test d'hypothèse vous a permis de déterminer qu'au moins un groupe diffère des deux autres. Il est à présent temps de réaliser un test de comparaisons multiples pour préciser où se situent les différences. Ce test de comparaison multiple doit être employé **si et seulement si** vous avez rejeté H0 dans le test de Kruskal-Wallis.
