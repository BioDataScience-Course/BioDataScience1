...
Dans ce tutoriel, nous allons utiliser R ensemble pour résoudre une question biologique à travail l'analyse de données. 
...
(biometry <- read("biometry", package = "BioDataScience1",
  lang = "fr"))
```
...
- Le jeu de données est disponible dans un package R\ : `BioDataScience1`, spécialement préparé pour ce cours.
...
Nous utilisons ici le chaînage (`%>.%`) et une sélection avec la fonctioin `select()` en éliminant des colonnes du tableau que nous n’utiliserons pas (les variables `day_birth`, `wrist` et `year_measure`).
...
```{r imc_mutate-check}
grade_code("La fonction `mutate()` n'a maintrenant plus de secrets pour vous.")
```
...
```{r case_when-check}
grade_code("C'est correct. Vous êtes maintenant capable de réaliser des cathégories avec `case_when()`.")
```
