...
BioDataScience1::config(debug = TRUE)
gradethis::gradethis_setup()
options(tutorial.event_recorder = learndown::record_learnr)
...
fixedRow(
  ...
  column(3, div(
    ...
    learndown::quitButton()
  ))
)
```

```{r, context="server"}
learndown::learnr_user_info(session, input, output, debug = TRUE)
learndown::trackQuit(session, input, output, delay = 10)
output$user <- renderText({learndown::user_name(input$user);""})
updateTextInput(session, "user", value = learndown::user_name())
```
...
```{r sea_temp1_h2, exercise=TRUE}

```

```{r sea_temp1_h2-hint-1}
...
```{r sea_temp1_h2-hint-2}
## Solution ##
13.5 + 0.053 * (2030 - 2020) 
```

```{r sea_temp1_h2-check}
grade_result(
