...
description: "**SDD I Module 11** ANOVAs à deux facteurs, modèles simple, complet et  hiérarchisé."
tutorial:
  id: "A11La_anova2"
  version: 2.1.0/11
output:
...
Lors de la réalisation du tutoriel précédent portant sur l'ANOVA à un facteur, vous avez étudié une variable réponse quantitative pour différents niveaux d'une variable qualitative (de type `factor` dans R). Dans le [module 11](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2021/variance2.html) du cours, vous avez vu qu'il était également possible de considérer deux variables facteurs avec une **ANOVA à deux facteurs**.
...
Assurez-vous d'avoir compris les sections [11.1](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2021/anova-%25C3%25A0-deux-facteurs.html), [11.2](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2021/mod%25C3%25A8le-sans-interactions.html) et [11.3](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2021/mod%25C3%25A8le-crois%25C3%25A9-complet.html) du cours. Vous devez également maîtriser les différentes notions vue dans le [module 10](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2021/variance.html) relatives à l'ANOVA à un facteur.
...
La question biologique à laquelle vous allez essayer de répondre est la suivante : **y a-t-il une différence de rendement dans la production de lait en fonction de la ration et du type d'alimentation donné aux vaches ?**
...
Réalisez sur le jeu de données `milk_production` le graphique permettant de visualiser les interactions entre les variables facteurs (`food` et `ration`). Commencez par calculer la moyenne du rendement par types d'alimentation et par ration.
...
___ %>.% # regroupement par type d'alimentation et par ration
  ___(., ___, ___) |> ___(milk_mean = ___(___)) %>.%  # moyenne du rendement
...
DF %>.% # regroupement par type d'alimentation et par ration
  group_by(., FACTOR1, FACTOR2) |> summarise(., milk_mean = mean(VARNUM)) %>.%  # moyenne du rendement
...
milk_production %>.% # regroupement par type d'alimentation et par ration
  group_by(., food, ration) |> summarise(milk_mean = mean(milk)) %>.% # moyenne du rendement
  collect_dtx(.) %>.% # collecter les résultats
...
grade_code("Ce graphique permet d'analyser les interactions entre les prédicteurs (rations et types d'alimentation) et la réponse (rendement laitier). Lorsque les lignes sont parallèles, il n'y a pas d'interactions. À l'inverse, lorsque les lignes ne sont pas parallèles, il existe probablement une ou plusieurs interactions. Moins les lignes sont parallèles plus l'interaction sera forte. Dans notre cas, nous serions tenté de penser qu'il n'y a pas d'intéractions. Cependant, il reste plus sûr de réaliser un modèle croisé complet dans un premier temps pour le vérifier. Nous pourrons alors nous rabattre sur un modèle croisé sans intéractions si cela se justifie.")
```
...
___ %>.%
  group_by(., ___, ___) |> summarise(
    mean = mean(___), sd = sd(___), count = sum(!is.na(___))) %>.%
  ___(.)
```

```{r lait_tab_h2-hint-1}
DF %>.%
  group_by(., XFACTOR1, XFACTOR2) |> summarise(
    mean = mean(YNUM), sd = sd(YNUM), count = sum(!is.na(YNUM))) %>.%
  collect_dtx(.)
...
milk_production %>.%
  group_by(., food, ration) |> summarise(
    mean = mean(milk), sd = sd(milk), count = sum(!is.na(milk))) %>.%
  collect_dtx(.)
```

```{r lait_tab_h2-check}
grade_code("Le tableau que vous avez obtenu est parfait pour présenter vos données avant de réaliser une ANOVA à deux facteurs. Attention, ce tableau est en anglais. N'oubliez pas de le traduire si votre rapport est en français.")
```
...
question("Au seuil alpha de 5%, y a-t-il homoscédasticité ?",
  ...
  correct = "Bravo, c'est correct. La valeur P que vous avez obtenue étant plus grande que le seuil alpha de 5%, vous ne rejetez donc pas H0 et pouvez considérer qu'il y a homoscédasticité.")
```
...
Comme pour l'ANOVA à un facteur, vous allez utiliser la fonction `lm()` qui demande un jeu de données et une formule avant d'appliquer la fonction `anova()` sur l'objet obtenu pour produire le tableau de l'analyse de la variance. Le modèle croisé avec interactions peut s'écrire de deux manières dans R.

```{r, echo=TRUE, eval=FALSE}
anova(anova. <- lm(data = DF, YNUM ~ XFACTOR1 + XFACTOR2 + XFACTOR1:XFACTOR2))
```

où `XFACTOR1:XFACTOR2` représente le terme correspondant aux interactions entre `XFACTOR1` et `XFACTOR2`.
...
Réalisez votre modèle ANOVA à deux facteurs avec interactions sur le rendement de lait en fonction du type d'alimentation et de la ration en utilisation la version simplifiée de la formule.
...
question("Y a-t-il des intéractions significatives entre les rations et les types d'alimentation au seuil alpha de 5% ?",
  ...
  correct = "Bravo, c'est correct. Il n'y a pas d'interactions entre les deux facteurs. Avant de continuez l'analyse, vous allez simplifier le modèle en retirant le terme correspondant aux interactions.")
```

### ANOVA à deux facteurs sans interactions

Lorsque vous n'avez pas d'interactions significatives, vous pouvez choisir de simplifier votre modèle en supprimant le terme correspondant. Votre nouveau modèle s'écrit donc de cette manière.
...
grade_code("Bravo, vous avez réussi à simplifier votre modèle. Dans cette analyse, les résidus suivent également une distribution Normale (vous faites comme si vous l'avez testé à l'aide d'un graphique quantile-quantile à ce stade). Les conditions de votre ANOVA à deux facteurs sont donc remplies. Vous pouvez interpréter vos résultats en répondant aux questions ci-dessous.")
```
...
```{r lait_prepa}
anova. <- lm(data = milk_production, milk ~ food + ration)
```
...
question("Quels sont les niveaux significativement différents au seuil alpha de 5% pour les types d'aliments ?",
  ...
  allow_retry = TRUE, correct = "Très bien ! En effet, la moyennes pour le maïs ensilé est différentes de celle du foin et de l'herbe.",
...
Les points sont proches de la droite, mise à part une légère dérive en haut à droite du graphique. Celle-ci étant légère, et l'ANOVA étant relativement robuste face à de petites déviations de la Normale, nous en concluons donc que les résidus du modèle ANOVA à deux facteurs sans interactions ne s'éloignent pas de manière problématique d'une distribution Normale. Nous pouvons considérer (avec le résultat complémentaire du test de Bartlett) que les conditions d'application sont suffisamment bien rencontrées. **Notez bien ici la nuance relative à la robustesse du modèle face à de petites dérives qui est importante !**.
