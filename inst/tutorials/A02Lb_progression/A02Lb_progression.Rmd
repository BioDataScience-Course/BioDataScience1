...
```{r, echo = TRUE}
biometry %>.%
  fselect(., -c(day_birth, wrist, year_measure)) %->%
  bio
```

**Notez ceci :**

Nous utilisons ici le chaînage (`%>.%`) et une sélection avec la fonction `fselect()` en éliminant des colonnes du tableau que nous n'utiliserons pas (les variables `day_birth`, `wrist` et `year_measure`).

Lorsqu'on utilise un chaînage d'instructions, l'assignation du résultat est plus lisible quand elle est réalisée à la fin (`-> bio`) et mêrme si possible, avec le nom à la ligne (comme dernière étape du pipeline). Notez l'utilisation ici l'assignation alternative `%->%` à n'utiliser que dans des cas très particuliers comme en fin de pipeline.
...
```{r, echo=TRUE}
bio %>.%
  fsubset(., gender == "W") %>.% # Sélectionne les femmes
  fselect(., - age) %>.% # Retire la colonne âge
  head(., n = 8) %>.% # Garde les huit premières lignes
  knitr::kable(., align = "c")
```
...
-   Le nuage de points nous montre la variation de la taille en fonction de la masse. (Notez au passage qu'ici l'indication `data`=\` est facultative, mais ce ne sera pas le cas avec toutes les fonctions).
...
Calculez de nouveau l'IMC et ajoutez vos résultats dans le tableau de données. Vous avez à votre disposition la fonction `fmutate()` qui requiert comme argument le jeu de données et le nom de la nouvelle variable comme nom d'argument suivi de la formule mathématique qui calcule son contenu.

```{r, echo=TRUE}
# Calculer l'IMC pour les femmes
woman <- fmutate(woman, bmi = weight / height^2)
...
# Calculer l'IMC pour les hommes
man <- fmutate(man, bmi = weight / height^2)
# Afficher le tableau de données
man
```

```{r imc_mutate-check}
grade_code("La fonction `fmutate()` n'a maintenant plus de secrets pour vous.")
```
...
Pour calculer l'IMC sur le jeu de données `bio_100`, nous employons à nouveau la fonction `fmutate()`.

```{r, echo=TRUE}
bio_100 <- fmutate(bio_100, bmi = weight / (height / 100)^2)
...
```{r bio_imc-solution}
# Calcul de l'IMC
biometry <- fmutate(biometry, bmi = weight / (height / 100)^2)
# Affichage des 5 premières lignes du tableau de données
head(biometry, n = 5)
```
...
```{r obese1-check}
grade_code("Vous apprenez vite. Les opérateurs de condition comme `==` sont très utiles en combinaison avec `fsubset()` pour sélectionner les lignes d'intérêt dans le tableau de données.")
```
